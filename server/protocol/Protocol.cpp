...
namespace protocol {
	...
	PACKET Protocol::encode(connectionMessage &message) const noexcept
	{
		...
		ptrBuffer = encodeCharArray(ptrBuffer, message.clientName,
			NAME_LENGTH);
	...
	PACKET Protocol::encode(callMessage &message) const noexcept
	{
		...
		ptrBuffer = encodeCharArray(ptrBuffer, message.clientName,
			NAME_LENGTH);
		ptrBuffer = encodeCharArray(ptrBuffer, message.contactName,
			NAME_LENGTH);
	...
	PACKET Protocol::encode(infoResponseMessage &message) const noexcept
	{
		...
		ptrBuffer = encodeCharArray(ptrBuffer, message.contactNames,
			strlen(message.contactNames));

		return packet;
	}

	messageType Protocol::getMessageType(PACKET_BUFFER &buffer
	) const noexcept
	{
	...
	connectionMessage Protocol::decodeConnectionMessage(
	...
	) const noexcept
	{
		...
		ptrBuffer = decodeCharArray(ptrBuffer, message.clientName,
			NAME_LENGTH);
	...
	callMessage Protocol::decodeCallMessage(PACKET_BUFFER &buffer
	) const noexcept
	{
		UINT8 *ptrBuffer = &buffer[0];
		callMessage message;
		...
		ptrBuffer = decodeCharArray(ptrBuffer, message.clientName,
			NAME_LENGTH);
		ptrBuffer = decodeCharArray(ptrBuffer, message.contactName,
			NAME_LENGTH);

		return message;
	}

	infoMessage Protocol::decodeInfoMessage(PACKET_BUFFER &buffer
	) const noexcept
	{
		UINT8 *ptrBuffer = &buffer[0];
		infoMessage message;
	...
	serverMessage Protocol::decodeServerMessage(PACKET_BUFFER &buffer
	) const noexcept
	{
		UINT8 *ptrBuffer = &buffer[0];
	...
	infoResponseMessage Protocol::decodeInfoResponseMessage(UINT8 *buffer,
		int length
	) const noexcept
	{
		UINT8 *ptrBuffer = &buffer[0];
		...
		if (length > 0) {
			...
			ptrBuffer = decodeCharArray(ptrBuffer,
				message.contactNames, length);
	...
	UINT8 *Protocol::encodeHeader(UINT8 *buffer, header &value
	) const noexcept
	{
	...
	UINT8 *Protocol::encodeCharArray(UINT8 *buffer, char *array,
		char length
	) const noexcept
	{
	...
	UINT8 *Protocol::encodeUShort(UINT8 *buffer, unsigned short &value
	) const noexcept
	{
		buffer[1] = value;
		buffer[0] = value >> 8;
	...
	UINT8 *Protocol::decodeHeader(UINT8 *buffer, header *value
	) const noexcept
	{
	...
	UINT8 *Protocol::decodeCharArray(UINT8 *buffer, char *array,
		char length
	) const noexcept
	{
		for (int i = 0; i < length; i++)
	...
	UINT8 *Protocol::decodeUShort(UINT8 *buffer, unsigned short *value
	) const noexcept
	{
