...
namespace protocol {
	...
	class Protocol {
		public:
			...
			messageType getMessageType(PACKET_BUFFER &
			) const noexcept;

			connectionMessage decodeConnectionMessage(
				PACKET_BUFFER &
			) const noexcept;

			callMessage decodeCallMessage(PACKET_BUFFER &
			) const noexcept;

			infoMessage decodeInfoMessage(PACKET_BUFFER &
			) const noexcept;

			serverMessage decodeServerMessage(PACKET_BUFFER &
			) const noexcept;

			infoResponseMessage decodeInfoResponseMessage(UINT8 *,
				int
			) const noexcept;
		...
		private:
			...
			UINT8 *encodeCharArray(UINT8 *, char *, char
			) const noexcept;

			UINT8 *encodeUShort(UINT8 *, unsigned short &
			) const noexcept;
			...
			UINT8 *decodeCharArray(UINT8 *, char *, char
			) const noexcept;

			UINT8 *decodeUShort(UINT8 *, unsigned short *
			) const noexcept;
	};
}
