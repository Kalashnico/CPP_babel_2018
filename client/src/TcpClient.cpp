...
namespace tcpclient {
	...
	protocol::infoResponseMessage TcpClient::receiveClients() noexcept
	{
		if (_socket.waitForReadyRead()) {
			...
			auto response = _protocol.decodeInfoResponseMessage(
				packet, 0);

			if (_socket.waitForReadyRead()) {
				auto received = _socket.readAll();
				protocol::UINT8 packet[protocol::PACKET_SIZE +
					response.nextMessageLength];
				for (int i = 0; i < protocol::PACKET_SIZE +
					response.nextMessageLength; i++)
					packet[i] = received.data()[i];

				return _protocol.decodeInfoResponseMessage(
					packet, response.nextMessageLength);
			}
		}
